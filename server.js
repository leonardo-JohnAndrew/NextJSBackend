    ...
    if(groupNumber.sim_id === null || groupNumber.sim_id === 0) { 
     ...
     const [prefix , number ] = message.split(":");
      if(prefix.toLowerCase() !== "p"){
          response = `Text must start in P: , \nexample: \nP:0-000-0\np:0-000-0` 
          return { 
         isInserted: false,
         response
          }  
      }
     const regex = /^\d+-\d+-\d+$/; // expects exactly 5 parts separated by dashes, all numeric sample: 12-23-34-45-56
     const isValidFormat = regex.test(number);
     if (!isValidFormat) {
        //response validation message must be\n Data:0-0-0-0-0
         response = `Invalid text format must be P:0-0-0 ,example:\nP:1-123-2\np:1-123-2`;
        return{
         isInserted: false,
         response
        };  
     ...
     if(parts[1]>999) { 
         response = `Invalid text middle part format should be a three-digit number, example:\nP:50-123-20\np:50-123-20`
         return { 
             isInserted: false,
             response
            }
...
// create combination patterns 001 002 -> 999
function generateCombinationPatterns() {
    const patterns = [];
    for (let i = 0; i <= 9; i++) {
        for (let j = 0; j <= 9; j++) {
            for (let k = 0; k <= 9; k++) {
...
async function insertPatternToDb() { 
    // get date of today 
